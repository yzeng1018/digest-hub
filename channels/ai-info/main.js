...
async function main() {
  console.log(`\n开始抓取 AI 情报（过去 ${cfg.timeWindowHours}h）…`);

  const [tweets, jike, blogs, podcasts] = await Promise.all([
    fetchTweets(sources.twitter, sources.nitterInstances, cfg),
    fetchJike(sources.jike, sources.rsshubInstances, cfg),
    fetchBlogs(sources.blogs, cfg),
    fetchPodcasts(sources.podcasts, cfg),
  ]);

  let articles = [...tweets, ...jike, ...blogs, ...podcasts];
  ...
  if (!args['no-score']) {
    ...
    await reportUsage('digest-hub/ai-info');
  } else {
