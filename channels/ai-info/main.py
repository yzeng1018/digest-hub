#!/usr/bin/env python3
"""每日 AI 情报 — 主入口"""

import argparse
import os
import sys
from datetime import datetime
from pathlib import Path

# 将 monorepo 根目录加入 Python 路径，使 common.* 可导入
sys.path.insert(0, str(Path(__file__).resolve().parents[2]))

from dotenv import load_dotenv
load_dotenv()

from fetcher         import fetch_all
from fetcher_podcast import fetch_podcasts
from enricher        import enrich_articles
from renderer        import render
from mailer          import send_digest
from config          import MAX_ARTICLES, DEDUP_THRESHOLD, SCORING_SYSTEM_PROMPT

from common.dedup     import deduplicate
from common.scorer    import score_articles, get_usage
from common.reporter  import report_to_gateway


def _podcast_summary_fn(art: dict) -> str:
    """播客用转录稿，普通内容用 summary[:300]。"""
    if art.get("platform") == "Podcast" and art.get("transcript"):
        return art["transcript"][:2000]
    return (art.get("summary") or "")[:300]


def main():
    parser = argparse.ArgumentParser(description="Daily AI digest")
    parser.add_argument("--no-score", action="store_true")
    parser.add_argument("--no-email", action="store_true")
    parser.add_argument("--output",   default="")
    args = parser.parse_args()

    articles = fetch_all() + fetch_podcasts()
    if not articles:
        print("未抓到任何内容，请检查网络或数据源。")
        sys.exit(1)

    articles = deduplicate(articles, DEDUP_THRESHOLD)

    if args.no_score:
        print("跳过评分 (--no-score)")
        for art in articles:
            art["score"] = 5
            art["reason_zh"] = ""
            art["title_zh"] = art["title"]
            art["summary_zh"] = art["summary"]
        usage_info = {}
    else:
        print(f"用 Qwen 对 {len(articles)} 条内容评分…")
        articles = score_articles(articles, SCORING_SYSTEM_PROMPT,
                                  batch_size=10, summary_fn=_podcast_summary_fn)
        usage_info = get_usage()
        report_to_gateway(usage_info, project="digest-hub/ai-info")

    articles.sort(key=lambda a: -a["score"])
    articles = articles[:MAX_ARTICLES]

    if not args.no_score:
        articles = enrich_articles(articles)
    else:
        for art in articles:
            art["background_zh"] = ""

    output_dir = Path(__file__).parent / "output"
    output_dir.mkdir(exist_ok=True)
    date_str    = datetime.now().strftime("%Y-%m-%d")
    output_path = args.output or str(output_dir / f"{date_str}.html")
    render(articles, output_path)

    if not args.no_email:
        send_digest(articles, usage_info=usage_info)

    must_reads = sum(1 for a in articles if a["score"] >= 8)
    print(f"\n✅ 完成。共 {len(articles)} 条 · 必读 {must_reads} 条")
    print(f"   HTML: file://{os.path.abspath(output_path)}")


if __name__ == "__main__":
    main()
